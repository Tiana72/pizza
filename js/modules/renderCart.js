...
const cartListItem = document.querySelector(".modal-cart__list");
const cartPriceItem = document.querySelectorAll(".modal-cart__price");
...
const renderCartList = async () => {
    ...
    if (cartControl.cartData.length) {
        cartListItem.textContent = "";
        nextBtn.disabled = false;
        const cardsData = await Promise.all(
            cartControl.cartData.map(
                async (item) =>
                    await getData(
                        `https://scratch-sunny-lilac.glitch.me/api/products/${item.id}`
                    )
            )
        );
        ...
        cartListItem.append(...cardsCart);
        
    } else {
        cartListItem.innerHTML = "<li>Корзина пуста...</li>";
        nextBtn.disabled = true;
    }
    cartPriceItem.forEach(elem => elem.textContent = `${totalPrice}  ₽`)
};
...
const submitOrder = async (e) => {
    ...
    try {
        const response = await fetch('https://scratch-sunny-lilac.glitch.me/api/orders/', {
            method: 'POST',
            headers: {
...
export const renderCart = () => {
    ...
    cartListItem.addEventListener('click', delCartItem);
